...
import type { AssetRecord, Campaign, Domain, MasterStoryboard, OperationalMaster, ReferenceSource, Template } from './types'
...
export default function App(){
  ...
  return <main style={brandToCssVars(demoBrand)}>
    ...
    <BriefWorkbench domains={domains} masters={masters} storyboards={storyboards} templates={templates} assets={assets} onOpen={setSelected}/>
    ...
    <footer><b>CIAPACOV Creative Engine v0.6 · Production Planning</b><span>{templates.length} templates · {masters.length} masters · {storyboards.length} storyboards · {assets.filter(a=>a.availability==='bundled').length} bundled assets · {campaigns.length} kits</span><span>Active Design System controls production identity.</span></footer>
  </main>
}
